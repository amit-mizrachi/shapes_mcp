    ...
    def _validate(self) -> FilterCondition:
        ...
        if self.operator == "IN":
            if not isinstance(self.value, list) or len(self.value) == 0:
                raise ValueError(f"IN operator requires a non-empty list for 'value'. Got: {self.value!r}")
        elif self.operator == "LIKE":
            if not isinstance(self.value, str):
                raise ValueError(f"LIKE operator requires a string 'value'. Got: {self.value!r}")
