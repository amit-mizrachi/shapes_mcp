...
class SqliteDataStore(DataStore):
    def __init__(self, database_path: Optional[str] = None, table_schema: TableSchema = None) -> None:
    ...
    async def select_rows(
        ...
        sql = f'SELECT {distinct_keyword}{select_columns}{computed_col} FROM "{self._table_schema.table_name}"{where_clause}{order_clause} LIMIT ?'
        params.append(limit)

        return await self._run_query(sql, params, count_sql, count_params)
    ...
    async def aggregate(
        ...
        params = case_params + where_params
        sql, params = self._build_aggregated_sql_query(
    ...
    @staticmethod
    def _collect_filter_params(filter_condition: FilterCondition, params: list) -> None:
        if filter_condition.operator in ("IN", "NOT IN"):
    ...
    @staticmethod
    def _normalize_group_by(group_by: Optional[str | list[str]]) -> list[str]:
        if group_by is None:
            return []
    ...
    async def _run_query(
        self, sql_query: str, params: list,
        count_sql: Optional[str] = None, count_params: Optional[list] = None,
    ) -> QueryResult:
        async with self._connection() as connection:
            try:
                ...
                return QueryResult(
                    ...
                    count=result.count, total_count=total_count,
                )
            except Exception:
