...
class SqliteDataStore(DataStore):
    def __init__(self, db_path: str | None = None, table_schema: TableSchema = None) -> None:
    ...
    async def aggregate(
        ...
        group_by_cols = self._normalize_group_by(group_by)
        ...
        if transform:
            ...
            validated_sql_operation = self._validate_aggregation_args(operation, field, group_by_cols)
            aggregation_expression = self._build_aggregation_expression(validated_sql_operation, field)
            case_params = []

        where_clause, where_params = self._build_where_clause(filters, filter_logic)
        having_clause, having_params = self._build_having_clause(having_operator, having_value, group_by_cols)

        params = case_params + where_params
        sql_query, params = self._build_aggregated_sql_query(
            aggregation_expression, where_clause, params, group_by_cols, limit, order_by, order,
    ...
    def _filter_to_sql_expression(self, filter_condition: FilterCondition) -> str:
        ...
        if filter_condition.operator in ("IN", "NOT IN"):
            ...
            return f'{col} {filter_condition.operator} ({placeholders})'
    ...
    def _validate_aggregation_args(self, operation: str, field: Optional[str], group_by_cols: list[str]) -> str:
    ...
    def _validate_transform_columns(self, transform: TransformExpression) -> None:
        ...
        for case in transform.cases:
            for fc in case.when:
                self._validate_column(fc.column)

    def _build_case_expression(self, transform: TransformExpression) -> tuple[str, list]:
        parts: list[str] = []
        params: list = []
        col = f'"{transform.source_column}"'

        for case in transform.cases:
            when_parts: list[str] = []
            for fc in case.when:
                when_parts.append(self._filter_to_sql_expression(fc))
                self._collect_filter_params(fc, params)

            condition = " AND ".join(when_parts)
            if case.then_multiply is not None:
                parts.append(f'WHEN {condition} THEN {col} * ?')
    ...
    def _build_having_clause(self, having_operator: Optional[str], having_value: Optional[float], group_by_cols: list[str]) -> tuple[str, list]:
        if having_value is None:
            return "", []
        if not group_by_cols:
            raise ValueError("having_operator/having_value require group_by to be set.")
        valid_having_ops = {"=", "!=", ">", ">=", "<", "<="}
        op = having_operator or ">="
        if op not in valid_having_ops:
            raise ValueError(f"having_operator must be one of {sorted(valid_having_ops)}, got: {op!r}")
        return f' HAVING {_AGG_RESULT_ALIAS} {op} ?', [having_value]

    def _build_aggregated_sql_query(self, aggregation_expression, where_clause, params, group_by_cols, limit, order_by, order, having_clause="", having_params=None):
        if not group_by_cols:
        ...
        group_by_sql = ", ".join(f'"{col}"' for col in group_by_cols)
        select_cols = ", ".join(f'"{col}"' for col in group_by_cols)

        sql_query = (
            f'SELECT {select_cols}, {aggregation_expression} AS {_AGG_RESULT_ALIAS} '
    ...
    def _collect_filter_params(self, fc: FilterCondition, params: list) -> None:
        if fc.operator in ("IN", "NOT IN"):
            params.extend(fc.value)
        elif fc.operator not in ("IS NULL", "IS NOT NULL"):
